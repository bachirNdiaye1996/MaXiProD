    ...
    if(isset($_POST['demandeApprobation'])){
        if(isset($_POST['check'])){
            for ($i = 0; $i < count($_POST['check']); $i++){
                $check=$_POST['check'][$i];
                $checks = explode("&&", $check);
                <?= $transfert['idtransfert'] ?>&&<?= $_GET['idtransfert'] ?>&&<?= $transfert['epaisseur'] ?>&&<?= $transfert['nbbobine'] ?>&&<?= $transfert['idmatierearrive'] ?>&&<?= $transfert['pointarrive'] ?>&&<?= $transfert['pointdepart'] ?>&&<?= $transfert['poidspese'] ?>&&<?php echo $_SESSION['nomcomplet'];?>">
                $idmatiere = $checks[0];         // idmatiere
                $idreception = $checks[1];       // .
                $epaisseur = $checks[2];         // .
                $nbbobine = $checks[3];          // Nombre bobine
