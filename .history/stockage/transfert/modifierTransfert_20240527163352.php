...
//La partie de recupe de l'id et script pour modifier
    ...
    if($_SERVER["REQUEST_METHOD"]=='GET'){
        ...
        $sql = "select * from transfertdetails where idtransfertdetail=$id";
        $result = $db->query($sql);
        $row = $result->fetch();
        while(!$row){
            header("location: transfert.php");
        ...
        if(isset($_POST['modifierTransfert'])){
            ...
            $codereception=$_POST['codereception'];
            $numbobine=$_POST['numbobine'];
            ...
            // |--> Pour le update des nombres de bobines dans matiere
                ...
                if($resultEpaisseur){
                    ...
                    // Enlever le nombre de bobine dans le lieu de depart
                        $req ="UPDATE matiere SET `nbbobineactuel` = `nbbobineactuel` - ? where `idmatiere`='$Iddepart';";
                    ...
                    // |--> Pour update transfert
                        $sql = "UPDATE `transfertdetails` SET `pointdepart` = '$pointdepart',`epaisseur` = '$epaisseur',`pointarrive` = '$pointarrive', `poidsdeclare` = '$poidsdeclare', `codereception` = '$codereception'
                        , `poidspese` = '$poidspese', `etatbobine` = '$etatbobine', `nbbobine` = '$nbbobine', `couleurhistorique`=2, `idmatieredepart` = '$Iddepart', `idmatierearrive` = '$Idarrive', `numbobine` = '$numbobine' WHERE `idtransfertdetail` = ?;";
                        //$result = $db->query($sql); 
                        $sth = $db->prepare($sql);    
                                ...
                                <div class="modal-body">
                                    <form action="#" method="POST" enctype="multipart/form-data" class="">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6 ">
                                                <div class="mb-6 mt-3 text-start">
                                                    <label class="form-label fw-bold" for="nom">Code réception</label>
                                                    <input class="form-control" type="number" name="codereception" value="<?php echo $row['codereception'];?>" placeholder="Taper le code de réception">
                                                </div>
                                            </div>  
                                            <div class="col-md-6 ">
                                                <div class="mb-6 mt-3 text-start">
                                                    <label class="form-label fw-bold" for="nom">Numéro fil machine</label>
                                                    <input class="form-control" type="number" name="numbobine" value="<?php echo $row['numbobine'];?>" placeholder="Taper le numéro du fil machine">
                                                </div>
                                            </div> 
                                            <div class="col-md-6">
                                                <div class="mb-6 mt-3 text-start">
