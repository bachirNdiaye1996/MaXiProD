...
// Pour insertion details reception
    if(isset($_POST['approbation'])){
        idmatiere=<?= $reception['idmatiere'] ?>&idreception=<?= $_GET['idreception'] ?>&epaisseur=<?= $reception['epaisseur'] 
                                                            ?>&nbbobine=<?= $reception['nbbobine'] ?>&lieutransfert=<?= $reception['lieutransfert'] ?>&&nomcomplet=<?php echo $_SESSION['nomcomplet'];?>
            for ($i = 0; $i < count($_POST['check']); $i++){
                $check=htmlspecialchars( $_POST['check'][$i]);
                $checks = explode("&&", $check);

                $heuredepartquartHeure = $checks[0]; 
                $heuredepartquartMin = $checks[1];
                $heuredepartquartHeure = $checks[0]; 
                $heuredepartquartMin = $checks[1];
                $heuredepartquartHeure = $checks[0]; 
                $heuredepartquartMin = $checks[1];
