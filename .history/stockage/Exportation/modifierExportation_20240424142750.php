...
//La partie de recupe de l'id et script pour modifier
    ...
    if($_SERVER["REQUEST_METHOD"]=='GET'){
        ...
        if(isset($_POST['modifierExportation'])){
            $id = $_GET['idexportationModifSimp'];
            ...
            */
            ...
            if(($nbEpaisseurDepart < $nbbobine) || ($nbbobine == 0)){
                ...
                //if(($pointdepart == "Cranteuse vers Metal1")){ // Vérifie le type de exportation
                ...
                // |--> Pour update exportation
                    $sql = "UPDATE `exportationdetails` SET `pointdepart` = '$pointdepart',`epaisseur` = '$epaisseur',`pointarrive` = '$pointarrive', `poidsdeclare` = '$poidsdeclare'
                    , `poidspese` = '$poidspese', `etatbobine` = '$etatbobine', `nbbobine` = '$nbbobine' WHERE `idexportationdetail` = ?;";
                    //$result = $db->query($sql); 
                    $sth = $db->prepare($sql);    
                    $sth->execute(array($id));
                // |--> Fin update exportation
                
                // Pour revenir à l'etat initial.
                    $sql1 = "UPDATE `exportationdetails` set `actifapprouvexportation`=0 where idexportationdetail=$_GET[idexportationModifSimp]";
                    $db->query($sql1);
                // Pour revenir à l'etat initial.
                ...
                // |--> Pour le update des nombres de bobines dans matiere
                    ...
                    //Rechercher le nombre de piéces sur le lieu de depart
                        $sqlEpaisseur = "SELECT * FROM `matiere` where `lieuexportation`='$pointdepart' and `epaisseur`='$epaisseur' and `nbbobineactuel` != 0 and `nbbobineactuel`>=$nbbobine LIMIT 1;";
                        // On prépare la requête
                        $queryEpaisseur = $db->prepare($sqlEpaisseur);
                    ...
                    //Rechercher le nombre de piéces sur le lieu d'arrive
                        $sqlEpaisseur = "SELECT * FROM `matiere` where `lieuexportation`='$pointarrive' and `poidsdeclare`='$poidsdeclare' and `epaisseur`='$epaisseur' LIMIT 1;";
                        // On prépare la requête
                        $queryEpaisseur = $db->prepare($sqlEpaisseur);
                                    ...
                                    <form action="#" method="POST" enctype="multipart/form-data" class="">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6 mt-3">
                                                <div class="mb-6 text-start">
                                                    <label class="form-label fw-bold" for="prenom" >Point d'arrivée</label>
                                                    <input class="form-control designa" type="number" value="<?php echo $row['pointarrive'];?>" name="pointarrive"  placeholder="Taper le point arrive ">
                                                </div>
                                            </div>
                                            <div class="col-md-6 mt-3">
                                                <div class="mb-6 text-start">
                                                    <label class="form-label fw-bold" for="prenom" >Poids déclare</label>
                                                    <input class="form-control designa" type="text" value="<?php echo $row['poidsdeclare'];?>" name="poidsdeclare" id="validationDefault02"  placeholder="Taper le poids déclaré" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 ">
                                                <div class="mb-6 mt-3 text-start">
                                            ...
                                            <div class="col-md-12 text-end">
                                                <div class="col-md-8 align-items-center col-md-12 text-end">
                                                    <?php if($error == "erreurEpaisseur"){ ?> 
                                                        <script>    
                                                            Swal.fire({
                                                                    text: 'Veiller revoir votre stockage (nombre de bobine, epaisseur ou les lieus de exportation) svp!',
                                                                    icon: 'error',
                                                                    timer: 5000,
                                                    ...
                                                    <div class="d-flex gap-2 pt-4">                           
                                                        <a class="btn btn-danger  w-lg bouton mr-3" href="detailExportation.php?idexportation=<?= $_GET['idexportation'] ?>">Annuler</a>
                                                        <input class="btn btn-success  w-lg bouton mr-3" name="modifierExportation" type="submit" value="Modifier">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
