...
$valideExportation="";
$idmatierearrive="";
$idexportation = $_GET['idexportation'];  // On recupére l'ID de la réception par get
    ...
    if(isset($_POST['approuveExportationRectifie'])){
        if(!empty($_POST['motifRectifier'])){
            ...
            //Rechercher le nombre de piéces sur le lieu d'arrive
                $sqlEpaisseur = "SELECT * FROM `matiere` where `lieutransfert`='$pointdepart' and `epaisseur`='$epaisseur' and `nbbobineactuel` != 0 and `nbbobineactuel`>=$nombrebobine LIMIT 1;";
            ...
            if($resultMatiereArrive){
                ...
                foreach($UserMails as $user => $item){
                    envoie_mail("Gestion de production réctification",$item['email'],"Demande de réctification pour l'exportation de code EXP00-$idexportation",$messageD);
                }
    
                
                header("location: detailExportation.php?idexportation=$idmatiereDemandeRectifier");
                exit;
            }else{
                       ...
                       <div class="col-lg-12">
                            <div class="card position-relative">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Liste des bobines exportées</h6>
                                </div>
                                <div class="row m-2">
                                                ...
                                                ?>
                                                    <tr>
                                                        <td style="<?php if($exportation['actifapprouvexportation'] == 1){ echo 'background-color:#D72708 ; color:white;';}else{echo 'background-color:#4e73df ; color:white;';}?>">
                                                            <a style="text-decoration: none; font-family: arial; font-size: 20px; color:white;" title="Allez vers la reception planifiée correspondante" href="" class="link-offset-2 link-underline"><?php echo "EXP00-".$exportation['idexportation']."-BOB-0".$exportation['idexportationdetail'] ?></a>
                                                        </td>
                                                            ...
                                                            $('#suprimerExportation<?= $i ?>').click(function(e) {
                                                                ...
                                                                confirmButtonText: "Suprimer la exportation",
                                                                    ...
                                                                    if (result.isConfirmed) {                                                                                                                  
                                                                        $.ajax({
                                                                                type: "POST",
                                                                                url: "supprimerExportation.php?idsupexportation=<?=  $_GET['idexportation'] ?>&idexportationsup=<?= $exportation['idexportationdetail'] ?>&epaisseur=<?= $exportation['epaisseur'] ?>&lieuDepart=<?= $exportation['pointdepart'] ?>&nombrebobine=<?= $exportation['nbbobine'] ?>",
                                                                                //data: str,
                                                                                success: function( response ) {
                                                                                    Swal.fire({
                                        ...
                                        ?>
                                            <div class="col-md-8 align-items-center">
                                                <div class="d-flex gap-2 pt-4">
                                                    ...
                                                    ?>
                                                        <a href="ajouterExportationFormulaire.php?idexportation=<?= $_GET['idexportation']?>&NombreLigne=0" class="btn btn-success w-lg bouton"><i class="fa fa-plus me-1"></i> Ajouter exportation</a>
                                                    <?php
                                                            //} 
                                                        }
                                                    ?>
