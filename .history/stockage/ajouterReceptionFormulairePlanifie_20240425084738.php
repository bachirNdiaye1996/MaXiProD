<?php
    ...
    if(isset($_POST['CreerReception'])){
        ...
        $dateplanifie=htmlspecialchars( $_POST['datereception']);
        $entetedf=htmlspecialchars( $_POST['entetedf']);
        $commentaire=htmlspecialchars($_POST['commentaire']);

        if(!empty($_POST['entetedf']) && !empty($_POST['datereception'])){
        
            $sql = "UPDATE `receptionplanifiee` SET `datereception` = '$dateplanifie', `commentaire` = '$commentaire', `entetedf` = '$entetedf' WHERE `idreception` = ?;";
            //$result = $db->query($sql); 
            $sth = $db->prepare($sql);    
            $sth->execute(array($idreception));

            //Pour l'insertion de l'entete de la DF dans la base reception
            $sql = "UPDATE `reception` SET `entetedf` = '$entetedf' WHERE `idreception` = ?;";
            //$result = $db->query($sql); 
            $sth = $db->prepare($sql);
            $sth->execute(array($idreception));
            ...
            for ($i = 0; $i < count($_POST['epaisseur']); $i++){
                if(!empty($_POST['epaisseur'][$i])){
                    ...
                    $poidsbrut=htmlspecialchars($_POST['poidsbrut'][$i]);
                    ...
                    // Ajouter le nombre d'epaisseur 
                        $epaisseur = $_POST['epaisseur'][$i];
                        $nbbobine = $_POST['nbbobine'][$i];
                    ...
                    if($nbbobine == 0){
                        ...
                        $insertUser=$db->prepare("INSERT INTO `matiereplanifie` (`idmatiereplanifie`, `epaisseur`, `poidsdeclare`, `dateajout`, `nbbobine`, `actif`, `lieutransfert`, `idreception`,`poidsbrut`) 
                        VALUES (NULL, ?, ?, current_timestamp(), ?, '1', ?, ?, ?);");
                        $insertUser->execute(array($epaisseur,$poidsdeclare,$nbbobine,'',$idreception,$poidsbrut));
                        ...
                        <body>
                            <div id='email-wrap' style='background: #3F5EFB; border-radius: 10px;'><br><br>
                                <p align='center' style='margin-top:20px;'>
                                    ...
                                    <p align='center' style='color:white'>$_SESSION[nomcomplet] a approuvé la demande d'autorisation de modifier ou de supprimer une réception de code de réception : <strong>REC00-$_GET[idexportationDemandeAccepter] </strong></p>
                        ...
                        foreach($UserMails as $user => $item){
                            envoie_mail("Gestion de production réctification",$item['email'],"Demande de réctification pour la réception de code de réception REC00-$_GET[idexportationDemandeAccepter]",$messageD);
                        }
            ...
            if($valideTransfert != "erreurInsertion"){
                header("location: detailsReceptionPlanifie.php?idreception=$idreception");
                exit;
            }
        }else{
            $valideTransfert="erreurInsertion";
        ...
        $(document).ready(function(){  
            ...
            <tr id="row'+i+'" class="rowClass">
                ...
                <td style="background-color:#CFFEDA ;">
                    <div class="col-md-10">
                        <div class="mb-1 text-start">
                            <input class="form-control designa" type="number" step="0.01" name="poidsbrut[]" id="validationDefault0<?$i+11?>" required>
                        </div>
                    </div>
                </td>
                                            ...
                                            <div class="row">
                                                <div class="col-md-2 ml-5 mr-5 mt-3">
                                                    <div class="mb-5 text-start">
                                                        ...
                                                        <input class="form-control" type="text" name="entetedf" value="<?= $row['entetedf'] ?>" id="validationDefault50" required placeholder="Mettez l'entete de la DF">
                                                    </div>
                                                </div>
                                                <div class="col-md-2 mt-3 ml-5">
                                                    <div class="mb-1 text-start">
                                                        <label class="form-label fw-bold" for="nom">Date de reception</label>
                                                        <input class="form-control" type="date" name="datereception" value="<?= $row['datereception'] ?>" id="validationDefault55" required>
                                                    </div>
                                                </div>
                                                            ...
                                                            ?>
                                                                <tr class="rowClass">
                                                                    ...
                                                                    <td style="background-color:#CFFEDA ;">
                                                                        <div class="col-md-10">
                                                                            <div class="mb-1 text-start">
                                                                                <input class="form-control" type="number" name="nbbobine[]" id="validationDefault04" required >
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td style="background-color:#CFFEDA ;">
                                                                        <div class="col-md-10">
                                                                            <div class="mb-1 text-start">
                                                                                <input class="form-control designa" type="number" step="0.01" name="poidsdeclare[]" id="validationDefault05" required >
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td style="background-color:#CFFEDA ;">
                                                                        <div class="col-md-10">
                                                                            <div class="mb-1 text-start">
                                                                                <input class="form-control designa" type="number" step="0.01" name="poidsbrut[]" id="validationDefault06" required >
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td style="background-color:#CFFEDA ;" class="text-center"> 
                                                ...
                                                <?php }else{  // Lorsqu'il y'a pas d'erreur ?>
                                                    <table class="table table-bordered" id="" width="100%" cellspacing="0">
                                                        <thead>
                                                            <tr>       
                                                                <th>Epaisseur</th>
                                                            ...
                                                            ?>
                                                                <tr class="rowClass">
                                                                    ...
                                                                    <td style="background-color:#CFFEDA ;">
                                                                        <div class="col-md-10">
                                                                            <div class="mb-1 text-start">
                                                                                <input class="form-control" type="number" name="nbbobine[]" value="<?php echo $_POST['nbbobine'][$i] ?>" id="validationDefault04" required >
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td style="background-color:#CFFEDA ;">
                                                                        <div class="col-md-10">
                                                                            <div class="mb-1 text-start">
                                                                                <input class="form-control designa" type="number" step="0.01" name="poidsdeclare[]" value="<?php echo $_POST['nbbobine'][$i] ?>" id="validationDefault05" required >
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td style="background-color:#CFFEDA ;">
                                                                        <div class="col-md-10">
                                                                            <div class="mb-1 text-start">
                                                                                <input class="form-control designa" type="number" step="0.01" name="poidsbrut[]" value="<?php echo $_POST['nbbobine'][$i] ?>" id="validationDefault06" required >
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td style="background-color:#CFFEDA ;" class="text-center"> 
                                                ...
                                                <div class="col-md-12 text-end">
                                                    <div class="col-md-8 align-items-center col-md-12 text-end">
                                                        ...
                                                        <div class="d-flex gap-2 pt-4">                           
                                                            <a href="detailsReceptionPlanifie.php?idreception=<?= $_GET['idreception'] ?>"><input class="btn btn-danger  w-lg bouton mr-3" name=""  value="Annuler"></a>
