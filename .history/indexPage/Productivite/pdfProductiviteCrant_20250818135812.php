<?php
  ...
  if($_SERVER["REQUEST_METHOD"]=='POST'){
    ...
    foreach($ProductionParControleur as $key  => $productionParControleur){
        if($i>0){   
            $pdf->Cell(38,5,utf8_decode($productionParControleur['diametre']),1,0,'C');
            $pdf->Cell(38,5,utf8_decode($productionParControleur['numerofin']),1,0,'C');
            $pdf->Cell(38,5,utf8_decode($productionParControleur['dechet']),1,1,'C');
        }
    }
