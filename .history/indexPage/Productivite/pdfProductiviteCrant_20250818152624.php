<?php
  ...
  if($_SERVER["REQUEST_METHOD"]=='POST'){
    ...
    $pdf->Cell(55,5,utf8_decode('Prenom et Nom opérateur'),1,0,'C');
    $pdf->Cell(55,5,utf8_decode('Poids produit par opérateur (KG)'),1,0,'C');
    $pdf->Cell(55,5,utf8_decode('Temps arret par opérateur'),1,1,'C');
    ...
    foreach($ProductionParControleur as $key  => $productionParControleur){
        if($i>0){   
            ...
            if(($TotalArret[0]-1) == 0){
                if($TotalArret[1] == 0){}else{$pdf->Cell(38,5,utf8_decode($TotalArret[1]." minutes"),1,1,'C');}
            }else{ 
                if($TotalArret[1] == 0){
                    $pdf->Cell(55,5,utf8_decode(($TotalArret[0]-1)." Heures "),1,1,'C');
                }else{
                    $pdf->Cell(55,5,utf8_decode(($TotalArret[0]-1)." Heures ".$TotalArret[1]." minutes"),1,1,'C');
                }
            }           
        }
        $i++;
