    ...
    if(isset($_POST['CreerFicheProduction'])){
        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart"){
            if($ProblemeArret == ""){
                ...
                $sql = "UPDATE `exportation` SET `dateexportation` = '$dateexportation', `transporteur` = '$transporteur', `commentaire` = '$commentaire', `saisisseur` = '$saisisseur' WHERE `idcranteuseq1` = ?;";
                //$result = $db->query($sql);
                $sth = $db->prepare($sql);    
