        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && ($ProblemeFicheExist != "erreurProblemeFicheExist")){            
            ...
            // Echantillons
                for ($i = 0; $i < count($_POST['Echandiametre']); $i++){   
                    ...
                    $poids=htmlspecialchars($_POST['Echanlongueur'][$i]);


                    $insertUser=$db->prepare("INSERT INTO `cranteuseechantillon` (`idcranteuseechantillon`, `echandiametre`, `echanlongueur`, `idfichecranteuseq1`, `actif`) 
                    VALUES (NULL, ?, ?, ?, '1');");
