        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && ($ProblemeFicheExist != "erreurProblemeFicheExist")){            
            ...
            // Consommations
                for ($i = 0; $i < count($_POST['diametre']); $i++){   
                    ...
                    $req ="UPDATE matiere SET `nbbobineactuel` = `nbbobineactuel` - 1 where `numbobine`='$numerofin';";
                    $reqtitre = $db->prepare($req);
            ...
            // Echantillons
                for ($i = 0; $i < count($_POST['Echandiametre']); $i++){   
                    ...
                    if($diametre != "" || $poids != "" || $df != ""){
                        ...
                        VALUES (NULL, ?, ?, ?, '1', ?);");
                        $insertUser->execute(array($diametre,$echanlongueur,$idfichecranteuseq1Max,$df));
                    }
                }
            //
