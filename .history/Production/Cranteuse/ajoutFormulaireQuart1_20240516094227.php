<?php
    ...
    if(isset($_POST['CreerFicheProduction'])){
        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart"){            
            $sql = "UPDATE `fichecranteuseq1` SET `dateCreation` = '$datecreationfiche', `compteurdebut` = '$compteurdebut', `compteurfin` = '$compteurfin', `controleur1` = '$controleur1', 
                `dateCreation` = '$datecreationfiche', `controleur2` = '$controleur2', `controleur3` = '$controleur3', `machine` = '$machine',
                `observationdebut` = '$observationdebut', `heuredepartquart` = '$heuredepartquart', `heurefinquart` = '$heurefinquart', `saisisseur` = '$user',
                `quart` = '$quart', `vitesse` = '$vitesse', `observationfin` = '$observationfin', `poidsestimetravaillenonnote` = '$poidsestimetravaillenonnote'
                WHERE `idcranteuseq1` = ?;";
            //$result = $db->query($sql);
            $insertUser=$db->prepare("INSERT INTO `fichecranteuseq1` (`idfichecranteuseq1`, `user`, `dateajout`, `quart`, `actif`, `dateCreation`, `compteurdebut`,
             `compteurfin`, `controleur1`, `controleur2`, `controleur3`, `machine`, `observationdebut`, `heuredepartquart`, `heurefinquart`, `saisisseur`, `vitesse`,
              `observationfin`, `poidsestimetravaillenonnote`, `accepteprodmodif`, `actifapprouvprod`) VALUES (NULL, 'user', current_timestamp(), 'quart', '1', 'date', 'comptdebut',
              'comptfin', 'cont1', 'cont2', 'cont3', 'machine', 'obserdebut', 'heurdeparqu', 'heurfinqu', 'sais', 'vites', 'obserfin', 'poidsestimetravaillenonnote', '0', '0');");
            $insertUser->execute(array($user,$quart,$dateCreation,$compteurdebut,$compteurfin,$controleur1,$controleur2,$controleur3,$machine,$observationdebut,$heuredepartquart,$heurefinquart,$user,$vitesse,$observationfin,$poidsestimetravaillenonnote));
            ...
            for ($i = 0; $i < count($_POST['debutarret']); $i++){
                ...
                $insertUser=$db->prepare("INSERT INTO `cranteuseq1arret` (`idcranteuseq1arret`, `debutarret`, `finarret`, `raison`, `idcranteuseq1`, `actif`) 
                VALUES (NULL, ?, ?, ?, ?, '1');");
                $insertUser->execute(array($debutarret,$finarret,$raisonerreur,$idcranteuseq1));
            }
            ...
            for ($i = 0; $i < count($_POST['diametre']); $i++){   
                ...
                $insertUser=$db->prepare("INSERT INTO `cranteuseq1consommation` (`idcranteuseq1consommation`, `diametre`, `numerofin`, `poids`, `dechet`, `codeReception`, `idcranteuseq1`, `actif`) 
                VALUES (NULL, ?, ?, ?, ?,?, ?, '1');");
                $insertUser->execute(array($diametre,$numerofin,$poids,$dechet,$codeReception,$idcranteuseq1));
            }

            header("location: detailsCranteuseQ1.php?idcranteuseq1=$idcranteuseq1&quart=1");
            exit;
        }
    }
