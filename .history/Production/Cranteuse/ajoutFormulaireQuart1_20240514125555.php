<?php
    ...
    if(isset($_POST['CreerFicheProduction'])){
        ...
        [] => Array ( [0] => 16:59 ) [] => LLSLSL [] => Array ( [0] => 4.80 ) [] => Array ( [0] => 14 ) [] => Array ( [0] => 17 ) 
        [] => Array ( [0] => 1800 ) [] => Mouhamadoul Bachir Ndiaye [] => 1 [] => 1 [] => 3 [] => 1420 
        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart"){            
            $sql = "UPDATE `cranteuseq1` SET `dateCreation` = '$datecreationfiche', `compteurdebut` = '$compteurdebut', `compteurfin` = '$compteurfin', `controleur1` = '$controleur1', 
            ...
            //$result = $db->query($sql);
            $sth = $db->prepare($sql);    
            $sth->execute(array($idcranteuseq1));
            ...
            for ($i = 0; $i < count($_POST['diametre']); $i++){   
                ...
                $finarret=htmlspecialchars($_POST['codeReception'][$i]);
