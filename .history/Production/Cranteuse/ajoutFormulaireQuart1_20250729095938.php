        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFilMachine != "erreurProblemeFilMachine" && ($ProblemeFicheExist != "erreurProblemeFicheExist")){            
            ...
            // Arrets
                for ($i = 0; $i < count($_POST['debutarret']); $i++){
                    ...
                    if(((strtotime($arretCranteuse['finarret']) - strtotime($debutarret)) > 0)){
                        $TotalArretH +=  (strtotime($arretCranteuse['finarret']) - strtotime($debutarret));
                    }else{
                        $TotalArretH +=  (-strtotime($debutarret) + strtotime($arretCranteuse['finarret']));
                    }
                    
                    $TotalHeureTravaillerelle = explode(":",date("H:i",$TotalArretH));
                    $TempsArretTotal[$i] = ($TotalHeureTravaillerelle[0]-1)." H ".$TotalHeureTravaillerelle[1]." min"; 
