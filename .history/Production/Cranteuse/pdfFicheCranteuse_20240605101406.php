<?php
    ...
    foreach($arrets as $arret){        
        ...
        if(((strtotime($arret['finarret']) - strtotime($arret['debutarret'])) > 0)){
            $TotalArretH +=  (strtotime($prodcrantq1['finarret']) - strtotime($arret['debutarret']));
        }else{
    ...
    $pdf->SetFont('Arial','B',10);
    $pdf->SetTextColor(50,60,100);
    $pdf->Cell(190,18,utf8_decode("Total arret : $TotalArretHReelH.' Heures ''.$TotalArretHReel[1].' minutes'          QUART : $Cranteuse[quart] / $Cranteuse[heuredepartquart] - $Cranteuse[heurefinquart]             CONTROLEURS : $Cranteuse[controleur1] */* $Cranteuse[controleur2]"),0,1,'C');
    $pdf->Cell(190,2,utf8_decode("HORAIRES : $Cranteuse[heuredepartquart]  -  $Cranteuse[heurefinquart]             OBSERVATIONS (DEBUT) : $Cranteuse[observationdebut]"),0,1,'C');
