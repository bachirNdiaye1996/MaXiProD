    ...
    if($_SERVER["REQUEST_METHOD"]=='GET'){
        ...
        if(isset($_POST['CreerFicheProduction'])){
            ...
            if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFicheExist != "erreurProblemeFicheExist" && $ProblemeFilMachine != "erreurProblemeFilMachine"){  
                ...
                // Inserer dans le log
                    $compteurdebut=htmlspecialchars( $rowEntete['compteurdebut']);
                    $compteurfin=htmlspecialchars( $rowEntete['compteurfin']);
                    $controleur1=htmlspecialchars( $rowEntete['controleur1']);
                    $controleur2=htmlspecialchars($rowEntete['controleur2']);
                    $machine=htmlspecialchars( $rowEntete['machine']);
                    $datecreationfiche=htmlspecialchars( $rowEntete['datecreationfiche']);
                    $observationdebut=htmlspecialchars($rowEntete['observationdebut']);
                    $heuredepartquart=htmlspecialchars($rowEntete['heuredepartquart']);
                    $heurefinquart=htmlspecialchars( $rowEntete['heurefinquart']);
                    $user=htmlspecialchars( $rowEntete['user']);
                    $quart=htmlspecialchars($rowEntete['quart']);
                    $vitesse=htmlspecialchars( $rowEntete['vitesse']);
                    $observationfin=htmlspecialchars($rowEntete['observationfin']);
                    $poidsestimetravaillenonnote=htmlspecialchars($rowEntete['poidsestimetravaillenonnote']);
                    ...
                    `compteurfin`, `controleur1`, `controleur2`, `machine`, `observationdebut`, `heuredepartquart`, `heurefinquart`, `saisisseur`, `vitesse`,
                     ...
                     ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', '0');");
                   $insertUser->execute(array($user,$quart,$datecreationfiche,$compteurdebut,$compteurfin,$controleur1,$controleur2,$machine,$observationdebut,$heuredepartquart,$heurefinquart,$user,$vitesse,$observationfin,$poidsestimetravaillenonnote,$idsupfiche));
