            ...
            if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart"){  
                ...
                // Productions
                    for ($i = 0; $i < count($_POST['Proddiametre']); $i++){   
                        ...
                        if($idConsommation != ""){
                            $insertUser=$db->prepare("UPDATE `cranteuseq1production` SET `diametre`=?, `numerofin`=?, `poids`=?,
                            `dechet`=?, `heuremontagebobine`=? where `idcranteuseq1production`=?;");
                        $insertUser->execute(array($diametre,$numerofin,$poids,$dechet,$heuremontagebobine,$idcranteuseq1production));
                        }else{
