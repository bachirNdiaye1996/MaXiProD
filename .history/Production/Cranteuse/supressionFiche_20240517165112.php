<?php
    ...
    if(isset($_GET['idsupfiche'])){
        ...
        $sql = "UPDATE `fichecranteuseq1` set `actif`=0, `couleurhistorique`=1, `actifapprouvprod`= 0, `accepteprodmodif`= 0 where `idfichecranteuseq1`=$idsupfiche";
        $db->query($sql);
        ...
        foreach($UserMails as $user => $item){
            envoie_mail("Gestion de production suppression fiche",$item['email'],"Suppression de la fiche de code CRANQ1-$idcranteuseq1",$messageD);
        }
