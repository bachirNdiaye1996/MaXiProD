<?php
    ...
    if(isset($_POST['CreerFicheProduction'])){
        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart"){            
            ...
            // Vérification de l'existance de l'état de production
                $sqlEpaisseur = "SELECT FROM `etatproduction` where `idfichecranteuseq1`=?";
                // On prépare la requête
                $queryEpaisseur = $db->prepare($idfichecranteuseq1Max);
                $queryEpaisseur->execute();
                $resultEtatProd = $queryEpaisseur->fetch();
            // Fin

            // Inserer dans etat production
                if(($resultEtatProd)){
                    if($machine == "Cranteuse 1"){
                    $insertUser=$db->prepare("UPDATE `etatproduction` SET `idficheproductioncranteuse1`=? where `idficheproductioncranteuse2`=?;");
                    $insertUser->execute(array($idfichecranteuseq1Max));
                    }else{
                        $insertUser=$db->prepare("UPDATE `etatproduction` SET `idficheproductioncranteuse2`=? where `idcranteuseq1arret`=?;");
                        $insertUser->execute(array($idfichecranteuseq1Max));
                    }
                }else{
                    if($machine == "Cranteuse 1"){
                        $insertUser=$db->prepare("INSERT INTO `etatproduction` (`idetatproduction`, `user`, `dateajout`, `idficheproductioncranteuse1`) 
                        VALUES (NULL, ?, ?, ?, ?);");
                        $insertUser->execute(array($user,$dateajout,$idfichecranteuseq1Max));
                    }else{
                        $insertUser=$db->prepare("INSERT INTO `etatproduction` (`idetatproduction`, `user`, `dateajout`, `idficheproductioncranteuse1`) 
                        VALUES (NULL, ?, ?, ?, ?);");
                        $insertUser->execute(array($user,$dateajout,$idfichecranteuseq1Max));
            ...
            for ($i = 0; $i < count($_POST['diametre']); $i++){   
                ...
                VALUES (NULL, ?, ?, ?, ?,?, ?, '1');");
                $insertUser->execute(array($diametre,$numerofin,$poids,$dechet,$codeReception,$idfichecranteuseq1Max));
            }
            ...
            for ($i = 0; $i < count($_POST['Proddiametre']); $i++){   
                ...
                $insertUser=$db->prepare("INSERT INTO `cranteuseq1production` (`idcranteuseq1production`, `proddiametre`, `prodnumerofin`, `prodpoids`, `proddechet`, `prodcodeReception`, `idfichecranteuseq1`, `actif`) 
                VALUES (NULL, ?, ?, ?, ?,?, ?, '1');");
                $insertUser->execute(array($diametre,$numerofin,$poids,$dechet,$codeReception,$idfichecranteuseq1Max));
            }
