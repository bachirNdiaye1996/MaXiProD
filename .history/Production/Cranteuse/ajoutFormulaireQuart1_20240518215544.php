    ...
    if(isset($_POST['CreerFicheProduction'])){
        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart"){            
            ...
            if(($resultEtatProd)){
                ...
                $insertUser=$db->prepare("INSERT INTO `etatproduction` (`idetatproduction`, `user`, `dateajout`, `idficheproduction`) 
                VALUES (NULL, ?, ?, ?, ?);");
                $insertUser->execute(array($user,$finarret,$raisonerreur,$idfichecranteuseq1Max));
            }
