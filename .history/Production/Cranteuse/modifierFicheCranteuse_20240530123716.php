                ...
                // Echantillons
                    for ($i = 0; $i < count($_POST['Echandiametre']); $i++){   
                        ...
                        if($diametre != "" || $poids != "" || $df != ""){
                            ...
                            if($idConsommation != ""){
                                $insertUser=$db->prepare("UPDATE `cranteuseechantillon` SET `diametre`=?, `poids`=?,
                                `idfichecranteuseq1Max`=?, `df`=? where `idcranteuseechantillon`=?;");
                            $insertUser->execute(array($diametre,$poids,$idfichecranteuseq1Max,$df,$idcranteuseechantillon));
                            }else{
