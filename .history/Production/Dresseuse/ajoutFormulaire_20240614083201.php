        ...
        if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFilMachine != "erreurProblemeFilMachine" && ($ProblemeFicheExist != "erreurProblemeFicheExist")){            
            ...
            // Arrets
                for ($i = 0; $i < count($_POST['debutarret']); $i++){
                    ...
                    $insertUser=$db->prepare("INSERT INTO `dresseusearret` (`idarret`, `debutarret`, `finarret`, `raison`, `idfichedresseuse`, `actif`) 
                    VALUES (NULL, ?, ?, ?, ?, '1');");
            ...
            // Productions
                for ($i = 0; $i < count($_POST['Proddiametre']); $i++){   
                    ...
                    $numerofin=$numerofinString[0];
                    $poids=htmlspecialchars($_POST['Prodpoids'][$i]);
                    $insertUser=$db->prepare("INSERT INTO `dresseuseproduction` (`iddresseuseproduction`, `proddiametre`, `prodnumerofin`, `prodpoids`, `idfichedresseuse`, `actif`,`prodnbBarreColis`,`prodnbcolis`,`prodnbbarrerestant`,`Proddechet`) 
                    VALUES (NULL, ?, ?, ?, ?, '1', ?, ?, ?, ?);");
                    $insertUser->execute(array($diametre,$numerofin,$poids,$idfichecranteuseq1Max,$ProdnbBarreColis,$Prodnbcolis,$Prodnbbarrerestant,$Proddechet));
                }
            //

            header("location: detailsDresseuse.php?idfichedresseuse=$idfichecranteuseq1Max&quart=$quart");
            exit;
        }
    }
