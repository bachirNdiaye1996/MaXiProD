                   ...
                   $insertUser->execute(array($Ruser,$Rquart,$Rdatecreationfiche,$Rcompteurdebut,$Rcompteurfin,$Rcontroleur1,$Rcontroleur2,$Rmachine,$Robservationdebut,$Rheuredepartquart,$Rheurefinquart,$Ruser,$Rvitesse,$Robservationfin,$Rpoidsestimetravaillenonnote,$idsupfiche));
                    ...
                    // Productions
                        foreach($rowProductions as $rowProduction => $key){
                            ...
                            $diametre=htmlspecialchars( $key['Proddiametre'][$i]);
                            $numerofinString = explode("/", htmlspecialchars($key['Prodnumerofin'][$i]));
                            $numerofin=$numerofinString[0];
                            $poids=htmlspecialchars($key['Prodpoids'][$i]);
                            $ProdnbBarreColis=htmlspecialchars($key['ProdnbBarreColis'][$i]);
                            $Prodnbcolis=htmlspecialchars($key['Prodnbcolis'][$i]);
                            $Prodnbbarrerestant=htmlspecialchars($key['Prodnbbarrerestant'][$i]);
                            $Prodlongueurbarre=htmlspecialchars($key['Prodlongueurbarre'][$i]);
                            $Proddechet=htmlspecialchars($key['Proddechet'][$i]);
                ...
                // Productions
                    for ($i = 0; $i < count($_POST['Proddiametre']); $i++){   
                        ...
                        if($idcranteuseq1production != ""){
                            ...
                            $insertUser=$db->prepare("INSERT INTO `dresseuseproduction` (`iddresseuseproduction`, `proddiametre`, `prodnumerofin`, `prodpoids`, `idfichedresseuse`, `actif`) 
                            VALUES (NULL, ?, ?, ?, ?, '1');");
