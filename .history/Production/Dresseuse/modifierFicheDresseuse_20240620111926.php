<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=='GET'){
        ...
        $sql = "select * from cranteuseq1arret where idfichedresseuse=$idsupfiche";
        $result = $db->query($sql);
        $rowErreurs = $result->fetchAll();

        $sql = "select * from cranteuseq1consommation where idfichedresseuse=$idsupfiche";
        $result = $db->query($sql);
        $rowConsommations = $result->fetchAll();

        $sql = "select * from cranteuseq1production where idfichedresseuse=$idsupfiche";
        $result = $db->query($sql);
        $rowProductions = $result->fetchAll();

        $sql = "select * from cranteuseechantillon where idfichedresseuse=$idsupfiche";
        $result = $db->query($sql);
            ...
            if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFicheExist != "erreurProblemeFicheExist" && $ProblemeFilMachine != "erreurProblemeFilMachine"){  
                ...
                // Inserer dans le log
                   ...
                   $insertUser->execute(array($Ruser,$Rquart,$Rdatecreationfiche,$Rcompteurdebut,$Rcompteurfin,$Rcontroleur1,$Rcontroleur2,$Rmachine,$Robservationdebut,$Rheuredepartquart,$Rheurefinquart,$Ruser,$Rvitesse,$Robservationfin,$Rpoidsestimetravaillenonnote,$idsupfiche));
                    ...
                    // Echantillons
                        foreach($rowEchantillons as $rowEchantillon => $key){
                            $diametre=htmlspecialchars($key['echandiametre']);
                            $echanlongueur=htmlspecialchars($key['echanlongueur']);
                            $df=htmlspecialchars($key['echandf']);
        
                            if($diametre != "" || $echanlongueur != "" || $df != ""){
                                $insertUser=$db->prepare("INSERT INTO `logcranteuseechantillon` (`idlogcranteuseechantillon`, `echandiametre`, `echanlongueur`, `idfichecranteuseq1`, `actif`, `echandf`) 
                                VALUES (NULL, ?, ?, ?, '1', ?);");
