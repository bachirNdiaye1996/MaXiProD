        ...
        if(isset($_POST['CreerFicheProduction'])){
            ...
            if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFicheExist != "erreurProblemeFicheExist" && $ProblemeFilMachine != "erreurProblemeFilMachine"){  
                ...
                for ($i = 0; $i < count($_POST['debutarret']); $i++){
                    ...
                    if(((strtotime($finarret) - strtotime($debutarret)) > 0)){
                        $TotalArretH +=  (strtotime($finarret) - strtotime($debutarret));
                    }else{
                        $TotalArretH +=  (-strtotime($debutarret) + strtotime($finarret));
                    }
                    
                    $TotalHeureTravaillerelle = explode(":",date("H:i",$TotalArretH));
                    $TempsArretTotal = ($TotalHeureTravaillerelle[0]-1)." H ".$TotalHeureTravaillerelle[1]." min"; 

                    if($idErreur != ""){
                        $insertUser=$db->prepare("UPDATE `dresseusearret` SET `debutarret`=?, `finarret`=?, `raison`=?, `dateCreation`=?, `duree`=? where `iddresseusearret`=?;");
                        $insertUser->execute(array($debutarret,$finarret,$raisonerreur,$datecreationfiche,$TempsArretTotal,$idErreur));
                        ...
                        $insertUser=$db->prepare("INSERT INTO `dresseusearret` (`iddresseusearret`, `debutarret`, `finarret`, `raison`, `idfichedresseuse`, `actif`, `dateCreation`, `duree`) 
                        VALUES (NULL, ?, ?, ?, ?, '1', ?);");
                        $insertUser->execute(array($debutarret,$finarret,$raisonerreur,$idsupfiche,$datecreationfiche,$TempsArretTotal));
                    }
                ...
                // Productions
                    for ($i = 0; $i < count($_POST['Proddiametre']); $i++){   
                        ...
                        if($iddresseuseproduction != ""){
                            ...
                            `prodlongueurbarre`=?, `proddechet`=?, `idfichedresseuse`=?, `dateCreation`=? where `iddresseuseproduction`=?;");
                            $insertUser->execute(array($diametre,$numerofin,$poids,$ProdnbBarreColis,$Prodnbcolis,$Prodnbbarrerestant,$Prodlongueurbarre,$Proddechet,$idsupfiche,$datecreationfiche,$iddresseuseproduction));
                                                                            ...
                                                                            <div class="col-md-10">
                                                                                <div class="mb-1 text-start">
                                                                                    <select class="form-control" name="numerofin[]" id="numerofin<?php echo $i; ?>" required>
                                                                                        <option value="<?php echo $rowConsommation['numerofin'].'/'.$rowConsommation['idcranteuseq1production'] ?>"><?php $numerofinString = explode("/", $rowConsommation['numerofin']); echo $numerofinString[0]; ?></option> 
                                                                                        <?php
                                                                                            foreach($stockCranteuse as $stock){
                                                                                        ?>       
                                                                                            <option value="<?php echo $stock['prodnumerofin'].'/'.$stock['idcranteuseq1production'].'/'.$stock['proddiametre'].'/'.$stock['prodpoids']; ?>"> <?php echo $stock['prodnumerofin']; ?></option>                                                                                     
