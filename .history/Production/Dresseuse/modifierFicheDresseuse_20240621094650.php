        ...
        if(isset($_POST['CreerFicheProduction'])){
            ...
            if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFicheExist != "erreurProblemeFicheExist" && $ProblemeFilMachine != "erreurProblemeFilMachine"){  
                   ...
                   $insertUser->execute(array($Ruser,$Rquart,$Rdatecreationfiche,$Rcompteurdebut,$Rcompteurfin,$Rcontroleur1,$Rcontroleur2,$Rmachine,$Robservationdebut,$Rheuredepartquart,$Rheurefinquart,$Ruser,$Rvitesse,$Robservationfin,$Rpoidsestimetravaillenonnote,$idsupfiche));
                    ...
                    // Productions
                        foreach($rowProductions as $rowProduction => $key){
                            ...
                            $insertUser=$db->prepare("INSERT INTO `logcranteuseproduction` (`idlogcranteuseproduction`, `proddiametre`, `prodnumerofin`, `prodpoids`, `idfichedresseuse`, `actif`) 
                            VALUES (NULL, ?, ?, ?, ?, '1');");
                ...
                for ($i = 0; $i < count($_POST['diametre']); $i++){   
                    ...
                    if($idConsommation != ""){
                        $insertUser=$db->prepare("UPDATE `dresseuseconsommation` SET `diametre`=?, `numerofin`=?, `poids`=?,
                        `dechet`=?, `heuremontagebobine`=? where `iddresseuseconsommation`=?;");
                       $insertUser->execute(array($diametre,$numerofin,$poids,$dechet,$heuremontagebobine,$idConsommation));
                    }else{
                        $insertUser=$db->prepare("INSERT INTO `dresseuseconsommation` (`iddresseuseconsommation`, `diametre`, `numerofin`, `poids`, `dechet`, `idfichedresseuse`, `actif`,`heuremontagebobine`) 
                        VALUES (NULL, ?, ?, ?, ?, ?, '1', ?);");
                ...
                // Productions
                    for ($i = 0; $i < count($_POST['Proddiametre']); $i++){   
                        ...
                        $Prodnumerofin=htmlspecialchars($_POST['Prodnumerofin'][$i]);
                        ...
                        if($idcranteuseq1production != ""){
                            $insertUser=$db->prepare("UPDATE `cranteuseq1production` SET `proddiametre`=?, `prodnumerofin`=?, `prodpoids`=?, `prodnbBarreColis`=?, `prodnbcolis`=?, `prodnbbarrerestant`=?,
                            `prodnumerofin`=?, `prodpoids`=?, `ProdnbBarreColis`=?, `Prodnbcolis`=?, `Prodnbbarrerestant`=?, `idfichecranteuseq1`=? where `idcranteuseq1production`=?;");
                                                                            ...
                                                                            <div class="col-md-10">
                                                                                <div class="mb-1 text-start">
                                                                                    <select class="form-control" name="numerofin[]" id="numerofin">
                                                                                        <option value="<?php echo $rowConsommation['numerofin'].'/'.$rowConsommation['diametre'].'/'.$rowConsommation['poids']; ?>"><?php $numerofinString = explode("/", $rowConsommation['numerofin']); echo $numerofinString[0]; ?></option> 
                                                                                        <?php
                                                                                            foreach($stockCranteuse as $stock){
                                                                                        ?>                                                                                        
                                                                            ...
                                                                            <div class="col-md-12">
                                                                                <div class="mb-1 text-start">
                                                                                    <select class="form-control" name="Prodnumerofin[]" id="numerofinProd">
                                                                                        <option value="<?php echo $rowProduction['prodnumerofin'].'/'.$rowProduction['proddiametre'].'/'.$rowProduction['prodpoids']; ?>"><?php $numerofinString = explode("/", $rowProduction['prodnumerofin']); echo $numerofinString[0]; ?></option> 
                                                                                        <?php
                                                                                            foreach($stockCranteuse as $stock){
                                                                                        ?>                                                                                        
                                                                                           <option value="<?php echo $stock['prodnumerofin'].'/'.$stock['proddiametre'].'/'.$stock['prodpoids']; ?>"> <?php echo $stock['prodnumerofin']; ?></option>                                                                                     
