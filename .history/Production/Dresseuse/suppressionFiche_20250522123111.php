<?php
    ...
    if(isset($_GET['idsupficheDresseuse'])){
        $idsupfiche = $_GET['idsupficheDresseuse'];
        $quart = $_GET['quart'];

        $sql = "UPDATE `fichedresseuse` set `actif`=0, `couleurhistoriqueSup`=1, `actifapprouvprod`= 0, `accepteprodmodif`= 0 where `idfichedresseuse`=$idsupfiche";
        $db->query($sql);

        $insertUser=$db->prepare("UPDATE `dresseuseproduction` SET `actif`=0 where `idfichedresseuse`=?;");
        $insertUser->execute(array($idsupfiche));
