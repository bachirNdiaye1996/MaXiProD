<?php
    ...
    // Variables
    $idfichedresseuse = $_GET['idlogfichedresseuse']; 
    $TotalPoids = 0;
    $TotalBarres = 0;

    //** Debut select de la production (arret)
        $sql = "SELECT * FROM `logdresseusearret` where `actif`=1 and `idfichedresseuse`=$idfichedresseuse ORDER BY `idlogdresseusearret` DESC;";
    ...
    //** Debut select de la production (consommation)
        $sql = "SELECT * FROM `logdresseuseconsommation` where `actif`=1 and `idfichedresseuse`=$idfichedresseuse ORDER BY `idlogdresseuseconsommation` DESC;";
    ...
    //** Debut select de la production (consommation)
        $sql = "SELECT * FROM `logdresseuseproduction` where `actif`=1 and `idfichedresseuse`=$idfichedresseuse ORDER BY `idlogdresseuseproduction` DESC;";
    ...
    //** Debut select de la production
        $sql = "SELECT * FROM `logfichedresseuse` where `actif`=1 and `idlogfichedresseuse`=$idfichedresseuse";
    ...
    foreach($productions as $production){ 
        ...
        $TotalPoids += (int)$production['prodpoids'];
        $TotalBarres += (int)($production['prodnbBarreColis']*$production['prodnbcolis']) + (int)$production['prodnbbarrerestant'];       
