            ...
            if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFicheExist != "erreurProblemeFicheExist" && $ProblemeFilMachine != "erreurProblemeFilMachine"){  
                ...
                for ($i = 0; $i < count($_POST['poidstotalconsommation']); $i++){  
                    ...
                    if(isset($_POST['numerofilmachine1'][$i])){
                        ...
                        $insertUser=$db->prepare("UPDATE `trefilageconsommation` SET `numerofilmachine`=?, `poidstotal`=?, `nouveaudiametre`=1, `finirfm` = 0, `diametre`=? where `idtrefilageconsommation`=?;");
                    ...
                    if(isset($_POST['numerofilmachine2'][$i])){
                        ...
                        if(isset($_POST['finirfm'][$i])){
                            ...
                            $insertUser=$db->prepare("UPDATE `trefilageconsommation` SET `numerofilmachine`=?, `poidstotal`=?, `finirfm` = 1, `nouveaudiametre`=0, `diametre`=? where `idtrefilageconsommation`=?;");
                            $insertUser->execute(array($numerofilmachine,$poidstotalconsommation,$diametre,$idtrefilageconsommation));
                        }else{
                            $insertUser=$db->prepare("UPDATE `trefilageconsommation` SET `numerofilmachine`=?, `poidstotal`=?, `finirfm` = 0, `nouveaudiametre`=0, `diametre`=? where `idtrefilageconsommation`=?;");
                            $insertUser->execute(array($numerofilmachine,$poidstotalconsommation,$diametre,$idtrefilageconsommation));
                        }
                    }
                }
