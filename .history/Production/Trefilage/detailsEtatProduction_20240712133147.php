...
/** Quart 1 */
    /** Cranteuse1 Q1 */
        ...
        foreach($productionTref as $productionTr => $Key){
            ...
            //** Debut select de la production (consommation)
                $sql = "SELECT sum(poids) as piodstotal, diametre, sum(dechet) as dechet FROM `trefilageproduction` where `actif`=1 and `fichetrefilage`=$idFicheTrefilage GROUP BY `diametre`;";
            ...
            //** Debut select de la production (arret)
                $sql = "SELECT * FROM `trefilagerevision` where `actif`=1 and `fichetrefilage`=$idFicheTrefilage;";
