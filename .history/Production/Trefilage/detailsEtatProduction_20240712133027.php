    ...
    /** Cranteuse1 Q1 */
        ...
        foreach($productionTref as $productionTr => $Key){
            ...
            //** Debut select de la production (consommation)
                $sql = "SELECT sum(poids) as piodstotal, diametre, sum(dechet) as dechet FROM `cranteuseq1consommation` where `actif`=1 and `fichetrefilage`=$IdproductioncrantC1Q1 GROUP BY `diametre`;";
            ...
            //** Debut select de la production (arret)
                $sql = "SELECT * FROM `cranteuseq1arret` where `actif`=1 and `fichetrefilage`=$IdproductioncrantC1Q1;";
