...
//** Debut select de la production
    $sql = "SELECT * FROM `ficheparquarttrefilage` where `actif`=1 ORDER BY `idficheparquarttrefilage` DESC";
...
if(isset($_POST['ajouterproductionTrefilage'])){
    $quart=htmlspecialchars($_POST['quart']);
    $nomchefquart=htmlspecialchars($_POST['nomchefquart']); 
    $nomchefsection=htmlspecialchars($_POST['nomchefsection']); 
    $dateCreation=htmlspecialchars($_POST['datecreation']);
    $nomchefequipe=htmlspecialchars($_POST['nomchefequipe']);
    $nompeseur=htmlspecialchars($_POST['nompeseur']); 
    $nombrepersonne=htmlspecialchars($_POST['nombrepersonne']);


    //On vérifie si la fiche existe ou pas
        $sqlEpaisseur = "SELECT * FROM `ficheparquarttrefilage` WHERE `actif`=1 AND `quart`='$quart' AND `dateCreation`='$dateCreation';";
        // On prépare la requête
        ...
        if($resultEpaisseur){
            ...
            $insertUser=$db->prepare("INSERT INTO `ficheparquarttrefilage` (`idficheparquarttrefilage`, `dateajout`, `datecreation`, `quart`, `nompeseur`, `nomchefquart`,
            `nombrepersonne`,`nomchefequipe`, `actif`,`nomchefsection`) VALUES (NULL, current_timestamp(), ?, ?, ?, ?, ?, ?, '1', ?);");
           $insertUser->execute(array($dateCreation,$quart,$nompeseur,$nomchefquart,$nombrepersonne,$nomchefequipe,$nomchefsection));
        
           header("location: ficheParQuartTrefilage.php?");
           exit;
        }
    // Fin
}
                                ...
                                <div class="table-responsive">
                                            ...
                                            ?>
                                                <tr>
                                                    <td style="<?php if($trefilage['actifapprouvprod'] == 0){ echo "background-color:#CFFEDA"; ?> <?php }else{ echo "background-color:#FF5335"; ?><?php }?>; text-align: center;" class="">
                                                        <a style="text-decoration: none; font-family: arial; font-size: 20px;" href="ficheTrefilage.php?idficheparquarttrefilage=<?= $trefilage['idficheparquarttrefilage'] ?>&quart=<?= $trefilage['quart'] ?>&dateCreation=<?= $trefilage['datecreation'] ?>" class="link-offset-2 link-underline"><?php echo "TREF-".$trefilage['idficheparquarttrefilage']; ?></a>
                                                    </td>
                                                    ...
                                                    <td style="<?php if($trefilage['actifapprouvprod'] == 0){ echo "background-color:#CFFEDA"; ?> <?php }else{ echo "background-color:#FF5335"; ?><?php }?>;">
                                                        <?= $trefilage['nomchefquart'] ?>
                                                    </td>
                                                    <td style="<?php if($trefilage['actifapprouvprod'] == 0){ echo "background-color:#CFFEDA"; ?> <?php }else{ echo "background-color:#FF5335"; ?><?php }?>;">
                                                        <?= $trefilage['nomchefsection'] ?>
                                                    </td>
                                                    <td style="<?php if($trefilage['actifapprouvprod'] == 0){ echo "background-color:#CFFEDA"; ?> <?php }else{ echo "background-color:#FF5335"; ?><?php }?>;">
                                                        <?= $trefilage['nomchefequipe'] ?>
                                                    </td>
                                                    <td style="<?php if($trefilage['actifapprouvprod'] == 0){ echo "background-color:#CFFEDA"; ?> <?php }else{ echo "background-color:#FF5335"; ?><?php }?>;">
                                                        <?= $trefilage['nombrepersonne'] ?>
                                                    </td>
                                                </tr>
                                            <?php
                                                }
                                    ...
                                    <div class="col-md-8 align-items-center mt-5">
                                        <div class="d-flex gap-2 pt-4">
                                            ...
                                            <div class="modal fade ajouterproductionTrefilage" id="" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
                                                <!-- Modale pour ajouter reception -->
                                                <div class="modal-dialog modal-xl modal-dialog-centered">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title font-weight-bold text-primary" id="myExtraLargeModalLabel">Ajouter une production</h5>
                                                                ...
                                                                <div class="row">
                                                                    ...
                                                                    <div class="col-md-6">
                                                                        <div class="mb-6 text-start">
                                                                            <label class="form-label font-weight-bold text-primary" for="prenom" >Date production</label>
                                                                            <input class="form-control designa" type="date" id="" value="<?php if(isset($_POST)){ echo $_POST['datecreation']; } ?>" name="datecreation" id="example"  placeholder="" required>
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-md-6 ">
                                                                        <div class="mb-6 mt-5 text-start">
                                                                            <label class="form-label font-weight-bold text-primary" for="1">Nom du peseur</label>
                                                                            <input class="form-control" type="text" name="nompeseur" value="<?php if(isset($_POST)){ echo $_POST['nompeseur']; } ?>" id="example-date-input4" placeholder="Taper le nom du peseur">
                                                                        </div>
                                                                    </div> 
                                                                    <div class="col-md-6">
                                                                        <div class="mb-6 mt-5 text-start">
                                                                            <label class="form-label font-weight-bold text-primary" for="2">Nom du chef de quart</label>
                                                                            <input class="form-control" type="text" name="nomchefquart" id="" value="<?php if(isset($_POST)){ echo $_POST['nomchefquart']; } ?>" id="example-date-input4" placeholder="Taper le nom du chef de quart" required>
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-md-6">
                                                                        <div class="mb-6 mt-5 text-start">
                                                                            <label class="form-label font-weight-bold text-primary" for="3" >Nom du chef d'équipe</label>
                                                                            <input class="form-control designa" type="text" id="" value="<?php if(isset($_POST)){ echo $_POST['nomchefequipe']; } ?>" name="nomchefequipe" id="example"  placeholder="Taper le nom du chef d'équipe" required>
                                                                        </div>
                                                                    </div> 
                                                                    <div class="col-md-6">
                                                                        <div class="mb-6 mt-5 text-start">
                                                                            <label class="form-label font-weight-bold text-primary" for="3" >Nom du chef de section</label>
                                                                            <input class="form-control designa" type="text" id="" value="<?php if(isset($_POST)){ echo $_POST['nomchefsection']; } ?>" name="nomchefsection" id="example"  placeholder="Taper le nom du chef de section" required>
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-md-6">
                                                                        <div class="mb-6 mt-5 text-start">
                                                                            <label class="form-label font-weight-bold text-primary" for="3" >Nombre de personne</label>
                                                                            <input class="form-control designa" type="text" id="" value="<?php if(!isset($_POST)){ echo $_POST['nombrepersonne']; } ?>" name="nombrepersonne" id="example"  placeholder="Taper le nombre de personne qui ont travaillé dans ce quart" required>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                <div class="">
                                                                    <div class="">
                                                                        <div class="d-flex gap-2 pt-5">   
                                                                            <?php if($ProblemeFicheExist == "erreurProblemeFicheExist"){ ?> 
                                                                                <script>    
                                                                                    Swal.fire({
                                                                                        text: 'Desolé mais cette fiche existe déja, veillez revoir les champs (Quart ou la Date) svp!',
                                                                                        icon: 'error',
                                                                                        timer: 4500,
                                                                            ...
                                                                            <input class="btn btn-success  w-lg bouton ml-4" name="ajouterproductionTrefilage" type="submit" value="Enregistrer">
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </form>  
