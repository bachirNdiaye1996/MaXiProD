    ...
    /** Trefilage Q1 */
        ...
        foreach($productionTref as $productionTr => $Key){
            ...
            //** Debut select de la production (consommation)
                ...
                $sql =
                "SELECT poidstotalproduction,produit
                FROM `trefilageproduction`
                INNER JOIN `trefilagerevision`
                ON trefilageproduction.idfichetrefilage = trefilagerevision.idfichetrefilage where trefilageproduction.actif=1 and trefilageproduction.idfichetrefilag e=$idFicheTrefilage;";
...
/*
$grouped_array = array();
foreach ($ProdConsommationQ1 as $prodRevision) {
...
*/
    ...
    //Pour quart 1
        ...
        $pdf->Cell(194,10,'',0,1);
        $pdf->SetTextColor(50,60,100);
        $pdf->Cell(190,8,utf8_decode("QUART I"),0,1,'C');
        $pdf->SetFont('Arial','B',7);

        $pdf->SetTextColor(0,0,0);
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(30,5,utf8_decode('Diametre'),1,0,'C');
        $pdf->Cell(30,5,utf8_decode('Objectif (KG)'),1,0,'C');

        // Production
        $pdf->SetTextColor(0,0,0);
        $pdf->SetFont('Arial','B',8);
        $pdf->Cell(30,5,utf8_decode('Production (KG)'),1,1,'C');

        $i=0;
        foreach($resultProdRevisionQ1 as $shipping => $values){ 
            $i++;       
            $pdf->Cell(30,5,utf8_decode($shipping),1,0,'C');
            if(count($resultProdRevisionQ1) > $i){
        ...
        $i=0;
        foreach($resultProdProductionQ1 as $shipping => $values){ 
            $i++;       
            if(count($resultProdProductionQ1) > $i){
                $pdf->Cell(30,5,utf8_decode($values['poidstotalproduction']),1,1,'C');
            }else{
                $pdf->Cell(30,5,utf8_decode($values['poidstotalproduction']),1,0,'C');
            }
        }
    //Fin quart 2
        ...
        $pdf->Cell(194,10,'',0,1);
        $pdf->SetTextColor(50,60,100);
        $pdf->Cell(190,8,utf8_decode("QUART II"),0,1,'C');
        $pdf->SetFont('Arial','B',7);
        ...
        $i=0;
        foreach($ProdConsommationQ1 as $shipping => $values){
            $i++;
            $pdf->Cell(30,5,utf8_decode($shipping),1,0,'C');
            if(count($ProdConsommationQ1) > $i){
                $pdf->Cell(30,5,utf8_decode($values['objectifminimal']),1,1,'C');
            }else{
                $pdf->Cell(30,5,utf8_decode($values['objectifminimal']),1,0,'C');
            }
        ...
        $i=0;
        foreach($resultProdProductionQ2 as $shipping => $values){ 
            $i++;
            if(count($resultProdProductionQ2) > $i){
                $pdf->Cell(30,5,utf8_decode($values['poidstotalproduction']),1,1,'C');
                $pdf -> Write(5,utf8_decode($values['poidstotalproduction']),'');
            }else{
                $pdf->Cell(30,5,utf8_decode($values['poidstotalproduction']),1,0,'C');
            }
        }
