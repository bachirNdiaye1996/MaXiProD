        ...
        if($ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFilMachine != "erreurProblemeFilMachine" && ($ProblemeFicheExist != "erreurProblemeFicheExist")){            
            ...
            // Productions
                for ($i = 0; $i < count($_POST['nombrerouleauxproduction']); $i++){   
                    ...
                    $insertUser=$db->prepare("INSERT INTO `trefilageproduction` (`iddresseuseproduction`, `proddiametre`, `prodnumerofin`, `prodpoids`, `idfichedresseuse`, `actif`,`prodnbBarreColis`,`prodnbcolis`,`prodnbbarrerestant`,`proddechet`,`prodlongueurbarre`) 
                    VALUES (NULL, ?, ?, ?, ?, '1', ?, ?, ?, ?, ?);");
