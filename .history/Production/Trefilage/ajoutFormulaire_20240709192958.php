        ...
        if($ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFilMachine != "erreurProblemeFilMachine" && ($ProblemeFicheExist != "erreurProblemeFicheExist")){            
            ...
            // Consommations
                for ($i = 0; $i < count($_POST['numerofilmachine']); $i++){   
                    ...
                    VALUES (NULL, ?, ?, '1', ?, ?);");
                    $insertUser->execute(array($numerofilmachine,$nombrerouleaux,$poidstotal,$idfichecranteuseq1Max));
                }
            //
            ...
            // Productions
                for ($i = 0; $i < count($_POST['nombrerouleauxproduction']); $i++){   
                    ...
                    VALUES (NULL, ?, ?, '1', ?);");
                    $insertUser->execute(array($nombrerouleauxproduction,$poidstotalproduction,$idfichetrefilage,$idfichecranteuseq1Max));
                }
            //

            header("location: detailsFicheTrefilage.php?idfichetrefilage=$idfichecranteuseq1Max&quart=$quart&idficheparquarttrefilage=$idficheparquarttrefilage");
            exit;
        }
    }
