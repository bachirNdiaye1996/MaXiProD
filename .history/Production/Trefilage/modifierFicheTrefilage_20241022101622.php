            ...
            if($ProblemeArret != "erreurProblemeArret" && $ProblemeCompteur != "erreurProblemeCompteur" && $ProblemeQuart != "erreurProblemeQuart" && $ProblemeFicheExist != "erreurProblemeFicheExist" && $ProblemeFilMachine != "erreurProblemeFilMachine"){  
                ...
                for ($i = 0; $i < count($_POST['poidstotalconsommation']); $i++){  
                    ...
                    if(isset($_POST['numerofilmachine1'][$i])){
                        ...
                        $insertUser=$db->prepare("UPDATE `trefilageconsommation` SET `numerofilmachine`=?, `poidstotal`=?, `nouveaudiametre`=0, `finirfm` = 0, `diametre`=? where `idtrefilageconsommation`=?;");
                    ...
                    if(isset($_POST['numerofilmachine2'][$i])){
                        ...
                        if(isset($_POST['finirfm'][$i])){
                            ...
                            $insertUser=$db->prepare("UPDATE `trefilageconsommation` SET `numerofilmachine`=?, `poidstotal`=?, `finirfm` = 1, `diametre`=? where `idtrefilageconsommation`=?;");
                            $insertUser->execute(array($numerofilmachine,$poidstotalconsommation,$diametre,$idtrefilageconsommation));
                        }else{
                        $insertUser=$db->prepare("UPDATE `trefilageconsommation` SET `numerofilmachine`=?, `poidstotal`=?, `finirfm` = 0, `diametre`=? where `idtrefilageconsommation`=?;");
                        $insertUser->execute(array($numerofilmachine,$poidstotalconsommation,$diametre,$idtrefilageconsommation));
                        }
                    }
                }
                                                                    ...
                                                                    <tr class="rowClass">
                                                                        ...
                                                                        <td style="background-color:#CFFEDA ;">
                                                                            <div class="col-md-10">
                                                                                <div class="form-check-inline">
                                                                                    <label class="form-check-label">
                                                                                        <input type="checkbox" style="width: 25px; height: 25px;" class="form-check-input" name="finirfm[]" <?php if(isset($_POST['finirfm'])){ echo "checked"; } ?> >
                                                                                    </label>
                                                                                </div>
                                                                            </div>  
                                                                        </td>
