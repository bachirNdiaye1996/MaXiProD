    ...
    if($_SERVER["REQUEST_METHOD"]=='GET'){
        ...
        if(isset($_POST['modifierTempsArret'])){
            for ($i = 0; $i < count($_POST['libelletempsarret']); $i++){   
                ...
                $insert=$db->prepare("UPDATE `cranteuseq1production` SET `prodnomenclature`=?, `proddiametre`=?, `prodnumerofin`=?, 
                `prodpoids`=?, `echanlongueur`=?, `echanpoids`=?, `echandf`=?, `idfichecranteuseq1`=? where `idcranteuseq1production`=?;");
                $insert->execute(array($nomenclature,$diametre,$numerofin,$poids,$echanlongueur,$echanpoids,$echandf,$idsupfiche,$idcranteuseq1production));
            }
        //
        }
    }
