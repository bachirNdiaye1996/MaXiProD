<?php
    ...
    if(isset($_POST['CreerReception'])){
        ...
        for ($i = 0; $i < count($_POST['epaisseur']); $i++){
            if(!empty($_POST['epaisseur'][$i])){
                if(!empty($_POST['epaisseur'][$i])){
                    ...
                    $lieutransfert=htmlspecialchars($_POST['machinereception'][$i]);


                    // Ajouter le nombre de bobine dans le lieu de reception
                        $insertUser=$db->prepare("INSERT INTO `receptionmachinedetails` (`idreceptionmachinedetails`, `poidsdeclare`, `poidspese`, `dateajout`,`user`,`lieutransfert`, `idreception`,`epaisseur`,`etatbobine`,`numbobine`,`df`) 
                        VALUES (NULL, ?, ?,current_timestamp(), ?, ?, ?, ?, ?, ?, ?);");
                        $insertUser->execute(array($poidsdeclare, $poidspese, $user, $lieutransfert, $idreception, $epaisseur, $etatbobine, $numbobine, $df));
        ...
        if($valideTransfert != "erreurInsertion"){
            header("location: detailsReception.php?idreception=$reception");
            exit;
        }
    }
