            ...
            if(!empty($_POST['epaisseur'][$i])){
                if(!empty($_POST['epaisseur'][$i])){
                    ...
                    // Ajouter le nombre de bobine dans le lieu de reception
                        $insertUser=$db->prepare("INSERT INTO `receptionmachinedetails` (`idreceptionmachinedetails`, `poidsdeclare`, `poidspese`, `dateajout`,`user`,`nbbobine`,`lieutransfert`, `idreception`,`epaisseur`,`etatbobine`,`numbobine`,`df`) 
                        VALUES (NULL, ?, ?, ?, current_timestamp(), ?, ?, ?, ?, ?, ?);");
                        $insertUser->execute(array($poidsdeclare, $poidspese, $user, $nbbobine, $lieutransfert, $idreception, $epaisseur, $etatbobine, $numbobine, $df));
